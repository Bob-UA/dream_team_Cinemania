import{r as i,c as _,d as k,b as B}from"./switcher-27652e0b.js";const d="MY_LIBRARY";let u={},c=null;i.libraryContainer.addEventListener("click",I);function f(e){if(e.genres){const{poster_path:t,title:o,id:s,genres:r,release_date:n}=e,a=r.map(g=>g.name),m=n?n.split("-")[0]:"";return`
  ...
  `}else if(e.genre_ids){const{poster_path:t,title:o,id:s,genre_ids:r,release_date:n}=e,a=R(B,r),m=Object.values(a),g=n?n.split("-")[0]:"";return`
  ...
  `;function b(e){e.currentTarget===e.target&&(i.backdropMovieInfoContainer.classList.add("is-hidden"),i.movieInfoContainer.innerHTML="",i.backdropMovieInfoContainer.removeEventListener("click",b))}function S(e){const t="Escape";e.code==t&&(i.backdropMovieInfoContainer.classList.add("is-hidden"),i.movieInfoContainer.innerHTML="",window.removeEventListener("keydown",S))}async function I(e){if(!e.target.classList.contains("js-modal-info"))return;i.backdropMovieInfoContainer.addEventListener("click",b),window.addEventListener("keydown",S),i.closeModalBtn.addEventListener("click",M),c=+e.target.dataset.id;const{data:o}=await O(c);u=o;const s=await _(u);i.movieInfoContainer.insertAdjacentHTML("beforeend",s),i.modal.classList.toggle("is-hidden");const r={addBtn:document.querySelector(".add-btn"),removeBtm:document.querySelector(".remove-btn")},n=p(d);n&&n.map(a=>{if(a.id===c){r.addBtn.classList.add("hide"),r.removeBtm.classList.remove("hide");return}}),r.addBtn.addEventListener("click",T),r.removeBtm.addEventListener("click",C)}function T(){const e={addBtn:document.querySelector(".add-btn"),removeBtm:document.querySelector(".remove-btn")},t=p(d);let o="",s=[];if(t&&c){s=h(t);const a=JSON.stringify(s);o=E(a)}const r=JSON.stringify(u),n=$(r,o);e.addBtn.classList.toggle("hide"),e.removeBtm.classList.toggle("hide"),L(d,n)}function C(){const e={addBtn:document.querySelector(".add-btn"),removeBtm:document.querySelector(".remove-btn")},t=p(d);e.addBtn.classList.toggle("hide"),e.removeBtm.classList.toggle("hide"),l=h(t),L(d,l);const o=i.libraryContainer.querySelector(".gallery-movies"),r=l.slice(0,v*y).map(n=>f(n)).join("");o.innerHTML=r}function h(e){return e.map(t=>{if(t.id!==c)return t}).filter(t=>{if(t)return t})}function p(e){try{const t=localStorage.getItem(e);return t===null?void 0:JSON.parse(t)}catch(t){console.error("Get state error: ",t.message)}}function L(e,t){try{const o=JSON.stringify(t);localStorage.setItem(e,o)}catch(o){console.error("Set state error: ",o.message)}}function E(e){return e.slice(1,e.length-1)}function $(e,t){return JSON.parse(t?`[${t}, ${e}]`:`[${e}]`)}function M(){i.modal.classList.toggle("is-hidden"),i.movieInfoContainer.innerHTML="",i.closeModalBtn.removeEventListener("click",M)}async function O(e){return await k(e)}function R(e,t){const o=[];return t.filter(s=>{e.map(({id:r,name:n})=>{r===s&&o.push(n)})}),o}
