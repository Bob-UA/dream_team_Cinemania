import{a as k,e as B}from"./switcher-d8a0ba9d.js";import{s as R}from"./catalog_hero-05b799bb.js";const b="9d709850c7590845ffb60644b29d6f51",h="https://api.themoviedb.org/3/",M="movie/upcoming",L="genre/movie/list",c="MY_LIBRARY";let m={};async function N(){const e=`${h}${M}?api_key=${b}`,s=await(await fetch(e)).json();if(s.results.length===0){O();return}const n=Math.floor(Math.random()*s.results.length),a=s.results[n];_(a)}function O(){const e=document.getElementById("movies-container");e.innerHTML="<p>OOPS... We are very sorry!But we couldn’t find the film.</p>"}async function _(e){const t=`${h}${L}?api_key=${b}`,n=await(await fetch(t)).json(),a=e.genre_ids.map(v=>{const f=n.genres.find(w=>w.id===v);return f?f.name:""}),{vote_average:i,vote_count:r,popularity:g,overview:u}=e,l=a.join(", "),p=document.getElementById("movies-container");p.innerHTML=`
    <div class="film-card">
